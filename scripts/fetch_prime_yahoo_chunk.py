...
def fetch(sym):
 ...
 for attempt in range(3):
  for host in HOSTS:
   try:
    u=f'https://{host}/v8/finance/chart/{sym}?range=2mo&interval=1d&events=history&includeAdjustedClose=false'
    with urllib.request.urlopen(urllib.request.Request(u,headers={'User-Agent':UA,'Accept':'application/json'}),timeout=20) as r: d=json.loads(r.read())
...
def main():
 ...
 sel=cs[x.chunk*x.chunk_size:(x.chunk+1)*x.chunk_size]; out=Path('runtime/prime_chunks');out.mkdir(parents=True,exist_ok=True); rows=[];errs=[]
 for c in sel:
  try:
   ...
   for i,t in enumerate(ts):
    ...
    d=datetime.fromtimestamp(int(t),timezone.utc).astimezone(TOKYO).strftime('%Y-%m-%d');rows.append([c['code'],c['company_name'],c['ticker_tse'],d,*v,u]);n+=1
 ...
 print(f'COMPLETE chunk={x.chunk} symbols={len(sel)}')
