...
JST=ZoneInfo('Asia/Tokyo'); LOOKBACK=20; VM=2.5
def main():
 today=datetime.now(JST).strftime('%Y-%m-%d'); cp=Path('data/constituents/tse_prime/current.csv')
 with cp.open(encoding='utf-8-sig',newline='') as f: cons=list(csv.DictReader(f))
 gathered={}; names={c['code']:c['company_name'] for c in cons}
 ...
 with (out/'audit_latest.csv').open('w',encoding='utf-8-sig',newline='') as f:w=csv.DictWriter(f,fieldnames=fields);w.writeheader();w.writerows(audit)
 sf=['trade_date','code','company_name','close','prior_20d_high','volume','prior_20d_avg_volume','volume_multiple']
 with (out/'latest.csv').open('w',encoding='utf-8-sig',newline='') as f:w=csv.DictWriter(f,fieldnames=sf);w.writeheader();w.writerows({k:r[k] for k in sf} for r in signals)
