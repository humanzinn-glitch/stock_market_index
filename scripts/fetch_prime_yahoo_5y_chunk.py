...
def main():
 ...
 sel=cs[x.chunk*x.chunk_size:(x.chunk+1)*x.chunk_size]; out=Path('runtime/prime_5y_chunks');out.mkdir(parents=True,exist_ok=True); rows=[];errs=[]
 for c in sel:
  try:
   ...
   if n<200:raise RuntimeError(f'rows={n}')
